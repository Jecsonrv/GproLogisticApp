...
const BillingWizard = ({
    ...
    useEffect(() => {
        if (isOpen && serviceOrder) {
            ...
            setFormData({
                ...
                issue_date: new Date().toISOString().split("T")[0],
                due_date: "",
                payment_condition: serviceOrder.client_payment_condition || "credito",
                notes: "",
            });
            fetchCharges();
        }
    ...
    const selectedTotals = useMemo(() => {
        ...
        charges.forEach(charge => {
            if (selectedChargeIds.includes(charge.id)) {
                subtotal += parseFloat(charge.amount); // changed from subtotal to amount for unified struct
                iva += parseFloat(charge.iva);         // changed from iva_amount to iva
    ...
    const handleCreateInvoice = async () => {
        if (selectedChargeIds.length === 0) {
            toast.error("Debe seleccionar al menos un cargo para facturar");
            return;
        }
        ...
        try {
            ...
            const selectedItems = charges.filter(c => selectedChargeIds.includes(c.id));
            const chargeIds = selectedItems.filter(c => c.type === 'service').map(c => c.original_id);
            const expenseIds = selectedItems.filter(c => c.type === 'expense').map(c => c.original_id);
            ...
            const msg = error.response?.data?.error || error.response?.data?.detail || "Error al crear la factura";
            toast.error(msg);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                <DialogHeader>
                    <DialogTitle>Generar Factura - {serviceOrder?.order_number}</DialogTitle>
                    <DialogDescription>
                        Seleccione los cargos a incluir y complete los datos fiscales.
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <div className="space-y-4">
                            {loading ? (
                                ...
                                <div className="text-center py-8 text-slate-500 bg-slate-50 rounded-lg">
                                    ...
                                    <p>No hay cargos pendientes de facturar para esta orden.</p>
                                </div>
                            ) : (
                                    ...
                                    <table className="min-w-full divide-y divide-slate-200 text-sm">
                                        <thead className="bg-slate-50">
                                            <tr>
                                                ...
                                                <th className="px-4 py-3 text-left font-semibold text-slate-600">Servicio / Descripción</th>
                                                <th className="px-4 py-3 text-right font-semibold text-slate-600">Monto</th>
                                                <th className="px-4 py-3 text-right font-semibold text-slate-600">IVA</th>
                                                <th className="px-4 py-3 text-right font-semibold text-slate-600">Total</th>
                                            </tr>
                                        </thead>
                                        <tbody className="bg-white divide-y divide-slate-100">
                                            {charges.map(charge => (
                                                <tr key={charge.id} className={selectedChargeIds.includes(charge.id) ? "bg-blue-50/30" : ""}>
                                                    ...
                                                    <td className="px-4 py-3 text-slate-900">
                                                        ...
                                                        {charge.notes && <div className="text-xs text-slate-500 truncate max-w-md">{charge.notes}</div>}
                                                        <div className="text-[10px] text-slate-400 uppercase mt-0.5">{charge.type === 'service' ? 'Servicio' : 'Gasto'}</div>
                                                    </td>
                                                    <td className="px-4 py-3 text-right tabular-nums text-slate-600">
                                                        {formatCurrency(charge.amount)}
                                                    </td>
                                                    <td className="px-4 py-3 text-right tabular-nums text-slate-600">
                                                        {formatCurrency(charge.iva)}
                                                    </td>
                                        ...
                                        <tfoot className="bg-slate-50 border-t border-slate-200">
                                            <tr>
                                                <td colSpan={2} className="px-4 py-3 text-right font-semibold text-slate-700">Total Seleccionado:</td>
                    ...
                    {step === 2 && (
                        <div className="space-y-6">
                            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 flex justify-between items-center">
                                <div>
                                    <p className="text-sm text-blue-700 font-medium">Monto a Facturar</p>
                                    <p className="text-2xl font-bold text-blue-800">{formatCurrency(selectedTotals.total)}</p>
                                </div>
                                <div className="text-right text-sm text-blue-600">
                                    {selectedChargeIds.length} cargos seleccionados
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 mt-1">
                                        Dejar vacío para generar <strong>PRE-00XXX-{new Date().getFullYear()}</strong>.
                                    </p>
                                </div>
                                <div>
