              ...
              {columns.map((column) => (
                <th
                  ...
                  className={`px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${
                    column.sortable !== false && column.accessor ? 'cursor-pointer hover:bg-gray-100' : ''
                  }`}
                >
                  <div className="flex items-center space-x-1">
                    <span>{column.header}</span>
                    {column.sortable !== false && column.accessor && sortColumn === column.accessor && (
                      <span>
                        {sortDirection === 'asc' ? (
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row, rowIndex) => (
                ...
                >
                  {columns.map((column) => (
                    <td key={column.accessor || column.header} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
      ...
      {showPagination && totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between">
          <div className="text-sm text-gray-700">
            ...
            <span className="font-medium">{Math.min(currentPage * pageSize, sortedData.length)}</span> de{' '}
