...
export const useTransfer = (id) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await axios.get(`/api/transfers/${id}/`);
      return data;
    },
    enabled: !!id,
  });
...
export const useCreateTransfer = () => {
  ...
  return useMutation({
    mutationFn: async (transferData) => {
      const { data } = await axios.post('/api/transfers/', transferData);
      return data;
    },
    onSuccess: () => {
...
export const useUpdateTransfer = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...transferData }) => {
      const { data } = await axios.patch(`/api/transfers/${id}/`, transferData);
      return data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['transfers'] });
