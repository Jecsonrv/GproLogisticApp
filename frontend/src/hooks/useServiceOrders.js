...
export const useServiceOrders = (filters = {}) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await axios.get('/api/orders/service-orders/', {
        params: filters,
      });
      return data;
    },
...
export const useServiceOrder = (id) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await axios.get(`/api/orders/service-orders/${id}/`);
      return data;
    },
    enabled: !!id,
  });
...
export const useCreateServiceOrder = () => {
  ...
  return useMutation({
    mutationFn: async (orderData) => {
      const { data } = await axios.post('/api/orders/service-orders/', orderData);
      return data;
    },
    onSuccess: () => {
...
export const useUpdateServiceOrder = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...orderData }) => {
      const { data } = await axios.patch(`/api/orders/service-orders/${id}/`, orderData);
      return data;
    },
    onSuccess: (data) => {
      // Invalidar cache de la lista y del item específico
      queryClient.invalidateQueries({ queryKey: ['service-orders'] });
      queryClient.invalidateQueries({ queryKey: ['service-orders', data.id] });
    },
  });
};
...
export const useDeleteServiceOrder = () => {
  ...
  return useMutation({
    mutationFn: async (id) => {
      await axios.delete(`/api/orders/service-orders/${id}/`);
      return id;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['service-orders'] });
