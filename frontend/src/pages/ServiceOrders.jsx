...
const ServiceOrders = () => {
  ...
  const fetchCatalogs = async () => {
    try {
      const [clientsRes, agentsRes, typesRes, providersRes] = await Promise.all([
        axios.get('/clients/'),
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.duca?.[0] || error.response?.data?.message || 'Error al crear orden';
  ...
  const handleCloseOrder = async (orderId) => {
    ...
    try {
      await axios.patch(`/orders/service-orders/${orderId}/`, { status: 'cerrada' });
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col gap-0.5">
            ...
            {row.bl_reference && (
                <span className="text-xs text-gray-500">BL: {row.bl_reference}</span>
            )}
        </div>
      )
    },
      ...
      render: (row) => (
        <div className="text-sm">
          ...
          <div className="flex gap-2 text-xs text-gray-400">
             ...
             <span>T: ${row.total_third_party?.toFixed(2) || '0'}</span>
          </div>
        </div>
      )
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col gap-1 items-start">
          <Badge variant={
            row.status === 'abierta' ? 'default' : 
            row.status === 'cerrada' ? 'secondary' : 'warning'
          }>
            {row.status === 'abierta' ? 'Abierta' : 
             row.status === 'cerrada' ? 'Cerrada' : row.status}
          </Badge>
          {row.facturado && (
             <Badge variant="outline" className="text-xs bg-blue-50 text-blue-700 border-blue-200">Facturado</Badge>
          )}
        </div>
      )
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Órdenes de Servicio</h1>
      ...
      >
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
                <Select
                    ...
                    onChange={(value) => setFormData({ ...formData, customs_agent: value })}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
                <Select
                    ...
                    onChange={(value) => setFormData({ ...formData, shipment_type: value })}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
                <Label className="mb-2 block">BL / Referencia</Label>
                <Input
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
                <Label className="mb-2 block">ETA (Fecha Estimada)</Label>
                <Input
                    type="date"
                    value={formData.eta}
            ...
            <div>
                <Label className="mb-2 block">PO (Purchase Order)</Label>
                <Input
